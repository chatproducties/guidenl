...
api: "GET https://app.chatwize.ai/api/v1/session/{uuid}"
...
```bash Curl
curl --location --request GET 'https://app.chatwize.ai/api/v1/session/{uuid}' \
...
uuid = '<session-uuid>'
url = f'https://app.chatwize.ai/api/v1/session/{uuid}'
headers = {
...
const url = `https://app.chatwize.ai/api/v1/session/${uuid}`;
const headers = {
